...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your backend-bd2 account.<br>
        ...
        &mdash; backend-bd2 Team
      `
      return sendMail({ toEmail: email, subject: 'backend-bd2 - Password Reset', content })
    })
